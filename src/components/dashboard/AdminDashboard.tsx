...
/* ─── Theme color system (brand/stone) ─── */
const ROLE_META: Record<string, { label: string; badge: string; barFrom: string; barTo: string }> = {
  ADMIN:          { label: 'Admin',          badge: 'bg-violet-100 text-violet-700 border-violet-200', barFrom: 'from-violet-500', barTo: 'to-violet-300' },
  RECRUITER:      { label: 'Recruiter',      badge: 'bg-brand-100 text-brand-700 border-brand-200',  barFrom: 'from-brand-500',  barTo: 'to-brand-300' },
  HIRING_MANAGER: { label: 'Hiring Manager', badge: 'bg-amber-100 text-amber-700 border-amber-200',  barFrom: 'from-amber-500',  barTo: 'to-amber-300' },
  INTERVIEWER:    { label: 'Interviewer',    badge: 'bg-sky-100 text-sky-700 border-sky-200',        barFrom: 'from-sky-500',    barTo: 'to-sky-300' },
  VIEWER:         { label: 'Viewer',         badge: 'bg-stone-100 text-stone-600 border-stone-200',  barFrom: 'from-stone-500',  barTo: 'to-stone-300' },
};
...
const STAGE_META: Record<string, { icon: React.ElementType; color: string; bg: string; border: string; glow: string }> = {
  Applied:    { icon: Layers,    color: 'text-brand-600',    bg: 'bg-brand-50',    border: 'border-brand-200',    glow: 'shadow-brand-500/10' },
  Screening:  { icon: Search,    color: 'text-violet-600',   bg: 'bg-violet-50',   border: 'border-violet-200',   glow: 'shadow-violet-500/10' },
  Interview:  { icon: Radio,     color: 'text-sky-600',      bg: 'bg-sky-50',      border: 'border-sky-200',      glow: 'shadow-sky-500/10' },
  Offer:      { icon: Zap,       color: 'text-amber-600',    bg: 'bg-amber-50',    border: 'border-amber-200',    glow: 'shadow-amber-500/10' },
  Hired:      { icon: UserCheck, color: 'text-emerald-600',  bg: 'bg-emerald-50',  border: 'border-emerald-200', glow: 'shadow-emerald-500/10' },
  Rejected:   { icon: UserX,     color: 'text-red-600',      bg: 'bg-red-50',      border: 'border-red-200',      glow: 'shadow-red-500/10' },
  Dropped:    { icon: Clock,     color: 'text-stone-600',    bg: 'bg-stone-50',    border: 'border-stone-200',    glow: 'shadow-stone-500/10' },
};
...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Users', value: users.length, icon: Users, accent: 'violet', sub: `${activeCount} active · ${inactiveCount} inactive`, trend: '' },
    { label: 'Open Positions', value: summary?.openPositions ?? 0, icon: Briefcase, accent: 'teal', sub: 'Active jobs posted', trend: '' },
    { label: 'Candidates', value: summary?.totalCandidates ?? 0, icon: TrendingUp, accent: 'sky', sub: 'In hiring pipeline', trend: '' },
    { label: 'Hire Rate', value: `${summary?.conversionRate ?? 0}%`, icon: BarChart3, accent: 'emerald', sub: 'Overall conversion', trend: '' },
  ];
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-7">
      {/* ── Hero Header ── */}
      <motion.div variants={fadeUp} className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-50 via-white to-brand-50/40 border border-stone-200/80 p-6 sm:p-8">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-violet-200/30 via-transparent to-transparent" />
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-stone-500 uppercase tracking-widest">System Overview</span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold text-stone-900 tracking-tight">Admin Console</h1>
            <p className="text-sm text-stone-500 mt-1.5">Real-time organisation health · <span className="text-stone-700 font-medium">{lastRefreshed.toLocaleTimeString()}</span></p>
          </div>
          <div className="flex items-center gap-2.5">
            <Link href="/dashboard/settings/users" className="flex items-center gap-2 px-4 py-2.5 bg-violet-600 text-white rounded-xl text-sm font-semibold hover:bg-violet-700 transition-all shadow-lg shadow-violet-500/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* ── Team Roles (wider) ── */}
        <motion.div variants={fadeUp} className="lg:col-span-2 rounded-2xl bg-white border border-stone-200/80 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500">Role distribution across {users.length} members</p>
              </div>
            </div>
            <Link href="/dashboard/settings/users" className="flex items-center gap-1 text-xs font-semibold text-violet-600 hover:text-violet-700 transition-colors">
              View all <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          </div>

          <div className="space-y-3">
            {(['ADMIN','RECRUITER','HIRING_MANAGER','INTERVIEWER','VIEWER'] as const).map(role => {
              const count = roleCount(role);
              const pct = users.length > 0 ? Math.round((count / users.length) * 100) : 0;
              const meta = ROLE_META[role];
              return (
                <div key={role} className="group flex items-center gap-4 p-2.5 -mx-2 rounded-xl hover:bg-stone-50 transition-colors">
                  <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase border ${meta.badge} w-[110px] text-center flex-shrink-0`}>
                    {meta.label}
                  </span>
                  ...
                  <span className="text-xs text-stone-400 w-9">{pct}%</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-stone-100">
            ...
            <div className="space-y-2">
              {users.slice(0, 5).map((u, idx) => {
                ...
                return (
                  <div key={u.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-stone-50 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-stone-800 truncate group-hover:text-stone-900">{u.name}</p>
                      <p className="text-xs text-stone-400 truncate">{u.email}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${meta?.badge ?? ROLE_META.VIEWER.badge}`}>{meta?.label ?? u.role}</span>
                      {!u.isActive && (
        ...
        <motion.div variants={fadeUp} className="rounded-2xl bg-white border border-stone-200/80 shadow-sm p-6">
          ...
          {logs.length === 0 ? (
            ...
            <div className="relative space-y-0">
              ...
              {logs.map((log, idx) => {
                ...
                return (
                  <div key={log.id} className="relative flex items-start gap-3.5 pb-5 last:pb-0 group">
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      <p className="text-sm font-semibold text-stone-800">{log.user.name}</p>
          ...
          <div className="mt-5 pt-4 border-t border-stone-100">
            <Link href="/dashboard/settings/audit-log" className="flex items-center justify-center gap-1.5 w-full py-2.5 rounded-xl text-xs font-semibold text-stone-600 bg-stone-50 hover:bg-stone-100 border border-stone-200 transition-colors">
      ...
      {summary?.pipeline && (
        <motion.div variants={fadeUp} className="rounded-2xl bg-white border border-stone-200/80 shadow-sm p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {summary.pipeline.filter(p => p.count > 0 || ['Applied','Screening','Interview','Offer','Hired','Rejected','Dropped'].includes(p.stage)).map((p, i) => {
              const meta = STAGE_META[p.stage] ?? STAGE_META.Dropped;
              const Icon = meta.icon;
              return (
                <motion.div key={p.stage} initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.1 + i * 0.06, duration: 0.4 }}
                  className={`relative rounded-2xl border ${meta.border} ${meta.bg} p-4 text-center hover:shadow-lg ${meta.glow} transition-all duration-300 group overflow-hidden`}>
                  <div className={`absolute bottom-0 left-0 right-0 h-[3px] ${meta.color.replace('text-', 'bg-')}`} />
                  <div className="relative">
                    <div className={`w-10 h-10 rounded-xl ${meta.bg} border ${meta.border} flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform`}>
                      <Icon className={`w-5 h-5 ${meta.color}`} />
                    </div>
                    <p className="text-2xl font-bold text-stone-900">{p.count}</p>
                    <p className={`text-xs font-semibold mt-1 ${meta.color}`}>{p.stage}</p>
                  </div>
                </motion.div>
              );
            })}
